...
% clear; clc; close all;
    ...
    fprintf('Function pwd = %s\n', pwd);
thisFile = mfilename('fullpath');
    [thisPath, ~, ~] = fileparts(thisFile);
    cd(thisPath);   % 保证 .fem / .ans 都在和脚本运行时同一个目录
...
[xs, ys] = pol2cart(45*pi/180, (cfg.r_outer+R_sy_out)/2);
...
% 4) 在齿区"挖槽 + 补两侧齿唇" %用来设计的定子内部不需要任何东西
group_bore = 11;
...
%     draw_arc_Rbore(R_bore, prev_p2_ang, th_q1, 5, 2);
% end
%  % mi_selectsegment(0.24, 31.5);
%  % mi_deleteselected;
...
theta0_deg = 0;               % 假设第一个 15° 就是 0~15°，否则填真实起始角
domain = prepare_design_domain(domain, theta0_deg);
    ...
    theta0_deg = 0;                 % 如果你的 0~90° 正好从全局 0° 开始
baseName   = 'apbc3';
femm_set_yoke_apbc_pairwise(domain, theta0_deg, baseName, maxsegdeg);
...
function femm_set_yoke_apbc_pairwise(domain, theta0_deg, baseName, maxsegdeg)
% FEMM_SET_YOKE_APBC_PAIRWISE
%   对定子轭 0° 和 90° 边界上的每一对小段，自动生成一组 periodic 边界
%
% domain.r_edges : 和画格子时用的是同一组半径
% theta0_deg     : 第一个 15° 扇区的起始角，一般就是 0
% baseName       : 边界基础名字，比如 'apbc3'
% maxsegdeg      : 你原来 mi_setsegmentprop 用的那个 maxsegdeg

    r_edges = domain.r_edges;
    nr      = numel(r_edges) - 1;

    % 左右两条边界的角度：theta0_deg 和 theta0_deg+90°
    th1 = theta0_deg;
    th2 = theta0_deg + 90;
    ...
    for ir = 1:nr
        ...
        % 本段的径向中点
        rmid = 0.5 * (r_edges(ir) + r_edges(ir+1));

        % 为这一对段生成独立的边界名字，例如 apbc3_01, apbc3_02, ...
        bndName = sprintf('%s_%02d', baseName, ir);

        % 先定义这条 periodic 边界属性
        % 对应你之前：mi_addboundprop('apbc3', 0,0,0, 0, 0, 0, 0, 0, 4);
        % 这里只是把名字换成 bndName
        mi_addboundprop(bndName, 0, 0, 0, 0, 0, 0, 0, 0, 4);  % 最后一个 4: Periodic/AntiPeriodic 类型

        % 左边界（0°）
        x1 = rmid * cosd(th1);  y1 = rmid * sind(th1);
        mi_selectsegment(x1, y1);
        mi_setsegmentprop(bndName, maxsegdeg, 1, 0, 0);
        mi_clearselected();

        % 右边界（90°）
        x2 = rmid * cosd(th2);  y2 = rmid * sind(th2);
